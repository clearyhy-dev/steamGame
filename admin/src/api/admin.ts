...
export const adminApi = {
  login: (username: string, password: string) =>
    unwrap(
      api.post<ApiEnvelope<{ token: string; username: string }>>("/api/admin/auth/login", {
        username,
        password,
      }),
    ),
  ...
  getDiscountProvidersSettings: () =>
    unwrap(
      api.get<ApiEnvelope<DiscountProvidersSettings>>("/api/admin/settings/discount-providers"),
    ),
  patchDiscountProvidersSettings: (body: Partial<DiscountProvidersSettings>) =>
    unwrap(
      api.patch<ApiEnvelope<DiscountProvidersSettings>>(
        "/api/admin/settings/discount-providers",
        body,
      ),
    ),

  getRuntimeSettings: () =>
    unwrap(api.get<ApiEnvelope<RuntimeSettingsResponse>>("/api/admin/settings/runtime")),
  patchRuntimeSettings: (
    body: Partial<RuntimeEffectiveSettings> & { steamAutoSyncEnabled?: boolean },
  ) => unwrap(api.patch<ApiEnvelope<RuntimeSettingsResponse>>("/api/admin/settings/runtime", body)),
  getRegionSettings: () =>
    unwrap(api.get<ApiEnvelope<RegionSettings>>("/api/admin/settings/region-settings")),
  ...
  patchSource: (sourceId: string, body: Record<string, unknown>) =>
    unwrap(
      api.patch<ApiEnvelope<{ sourceId: string }>>(`/api/admin/video-sources/${sourceId}`, body),
    ),

  ingestSource: (sourceId: string) =>
    unwrap(
      api.post<ApiEnvelope<{ videoId: string; jobId?: string }>>(
        `/api/admin/video-sources/${sourceId}/ingest`,
      ),
    ),

  getSource: (sourceId: string) =>
    unwrap(api.get<ApiEnvelope<VideoSourceRow>>(`/api/admin/video-sources/${sourceId}`)),
  ...
  videoDetail: (videoId: string) =>
    unwrap(
      api.get<ApiEnvelope<{ video: VideoRow; source: VideoSourceRow | null }>>(
  ...
  steamGames: (params?: {
    ...
    unwrap(
      api.get<ApiEnvelope<{ total: number; rows: SteamGameRow[] }>>("/api/admin/steam-games", {
        params,
      }),
    ),
  ...
  games: (params?: {
    ...
    unwrap(
      api.get<
        ApiEnvelope<{ total: number; page: number; pageSize: number; rows: GameManageRow[] }>
  ...
  syncAppList: (body?: { chunkSize?: number; lastAppId?: number; maxResults?: number }) =>
    unwrap(
      ...
      >("/api/admin/games/sync-app-list", body ?? {}),
    ),

  syncGameDetail: (appid: string) =>
    unwrap(
      api.post<ApiEnvelope<{ synced: boolean; appid: string }>>(
        `/api/admin/games/${appid}/sync-detail`,
      ),
    ),
  syncGameDeals: (
    appid: string,
    ...
    unwrap(
      api.post<
        ApiEnvelope<{
          appid: string;
          upserted: number;
          writeStats?: { inserted: number; updated: number; deduped: number };
          providers?: Array<{ source: string; ok: boolean; reason?: string }>;
        }>
      >(`/api/admin/games/${appid}/sync-deals`, body ?? {}),
    ),
  syncGameDealsBatch: (body?: {
    appids?: string[];
    batchSize?: number;
    delayMs?: number;
    cursorAppid?: string;
    ...
    unwrap(
      ...
      >("/api/admin/games/sync-deals-batch", body ?? {}),
    ),
  syncGameDealsHotTop: (body?: {
    topN?: number;
    delayMs?: number;
    sources?: Array<"steam" | "isthereanydeal" | "ggdeals" | "cheapshark">;
    ...
    unwrap(
      ...
      >("/api/admin/games/sync-deals-hot-top", body ?? {}),
    ),

  syncGameDetailsBatch: (body?: {
    appids?: string[];
    batchSize?: number;
    delayMs?: number;
    offset?: number;
    ...
    unwrap(
      ...
      >("/api/admin/games/sync-details", body ?? {}),
    ),

  gameSyncJobs: (params?: { limit?: number }) =>
    unwrap(
      api.get<ApiEnvelope<{ rows: SteamSyncJobRow[] }>>("/api/admin/games/sync-jobs", { params }),
    ),
  ...
  syncGameMeta: (appid: string) =>
    unwrap(
      api.post<ApiEnvelope<{ synced: boolean; appid: string }>>(
        `/api/admin/games/${appid}/sync-meta`,
      ),
    ),
  ...
  patchGame: (appid: string, body: { discountUrl: string }) =>
    unwrap(
      api.patch<ApiEnvelope<{ appid: string; discountUrl: string }>>(
        `/api/admin/games/${appid}`,
        body,
      ),
    ),
  ...
  createGameDealLink: (
    ...
    unwrap(
      api.post<ApiEnvelope<{ deal: DealLinkRow }>>(`/api/admin/games/${appid}/deal-links`, body),
    ),
  ...
  patchGameDealLink: (
    ...
    unwrap(
      api.patch<ApiEnvelope<{ deal: DealLinkRow }>>(
        `/api/admin/games/${appid}/deal-links/${dealId}`,
        body,
      ),
    ),

  youtubeVideos: (params?: { channelTitle?: string; liveOnly?: boolean }) =>
    unwrap(api.get<ApiEnvelope<YouTubeVideoRow[]>>("/api/admin/youtube-videos", { params })),
};
