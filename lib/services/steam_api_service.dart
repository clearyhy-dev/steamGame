...
class SteamApiService {
  ...
  Future<List<GameModel>> fetchFreeDeals({int pageSize = 30}) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (data is List) {
          return data
              .map((e) => GameModel.fromCheapSharkDeal(
  ...
  Future<GameModel?> fetchSteamAppDetails(String appId,
    ...
    try {
      ...
      if (appData is! Map<String, dynamic> || appData['success'] != true)
        return null;
  ...
  Future<List<GameModel>> fetchTopGamesByCurrentPlayers(
      {int limit = 15, String? country}) async {
    ...
    for (var i = 0; i < ids.length; i += batchSize) {
      ...
      final details = await Future.wait(
          batch.map((id) => fetchSteamAppDetails(id, country: country)));
    ...
    list.sort((a, b) => (CurrentPlayersCache.get(b.steamAppID) ?? 0)
  ...
  Future<GameModel> fetchGameDetail(String appId, {String? country}) async {
    return fetchGameById(appId, country: country);
  }
  ...
  Future<List<String>> fetchSteamScreenshots(String steamAppID,
      {String? country}) async {
  ...
  static Map<String, dynamic> parseReview(Map<String, dynamic> r) {
    ...
    final created = ts is int
        ? ts
        : (ts is num ? ts.toInt() : int.tryParse(ts?.toString() ?? '0') ?? 0);
    final updated = r['timestamp_updated'];
    final updatedSec = updated is int
        ? updated
        : (updated is num
            ? updated.toInt()
            : int.tryParse(updated?.toString() ?? '0') ?? 0);
    return {
      'content':
          r['review']?.toString().replaceAll(RegExp(r'<[^>]*>'), '').trim() ??
              '',
      'author': authorStr,
      'votes_up': (r['votes_up'] is int)
          ? r['votes_up'] as int
          : int.tryParse(r['votes_up']?.toString() ?? '0') ?? 0,
  ...
  Future<({List<Map<String, dynamic>> reviews, Map<String, dynamic>? summary})>
      fetchSteamReviews(
    String steamAppID, {
    int latestN = 10,
  }) async {
    ...
    try {
      final uri =
          Uri.parse('https://store.steampowered.com/appreviews/$steamAppID')
              .replace(
        queryParameters: <String, String>{
          'json': '1',
          'language': 'all',
  ...
  List<Map<String, dynamic>> getPriceHistoryPoints(
      double currentPrice, double originalPrice,
      {int lastChangeSec = 0}) {
  ...
  Future<List<Map<String, dynamic>>> fetchPriceHistoryFromItad(
      String steamAppID) async {
    ...
    try {
      ...
      final list =
          historyData['data'] ?? historyData['list'] ?? historyData['history'];
      if (list is! List || list.isEmpty) return [];

      final points = <Map<String, dynamic>>[];
      for (final e in list) {
        if (e is! Map<String, dynamic>) continue;
        final priceVal = (e['price'] is num)
        ...
        if (tMs == 0 && e['ts'] != null)
          tMs = (e['ts'] is int)
              ? (e['ts'] as int) * 1000
              : (int.tryParse(e['ts']?.toString() ?? '0') ?? 0) * 1000;
        if (tMs == 0) continue;
      ...
      if (response.statusCode == 200) {
        ...
        if (data is List) {
          ...
          for (final e in data) {
            ...
            final titleStr = m['external']?.toString() ??
                m['internalName']?.toString() ??
                '';
