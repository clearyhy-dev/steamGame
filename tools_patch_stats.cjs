...
"",
"async function steamStoreGenres(appid) {",
"  try {",
...
"    const genres = row.data && row.data.genres;",
"    if (!Array.isArray(genres)) return [];",
"    return genres.map((g) => String((g && g.description) || '').trim()).filter(Boolean);",
"  } catch {",
"    return [];",
"  }",
"}",
"",
"async function inferFavoriteGenres(topAppIds, maxCalls) {",
...
"    const steamId = user && user.steamId && user.steamId.trim();",
"    if (!steamId) {",
"      return {",
"        steamLinked: false,",
"        ownedCount: 0,",
...
fs.writeFileSync("d:/googleplay/steamgame/steamGame/server/src/modules/stats/stats.service.ts", lines.join("\n") + "\n");
